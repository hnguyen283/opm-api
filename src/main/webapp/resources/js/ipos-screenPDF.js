...
window.Feedback = function( options ) {
    ...
    // default properties
    options.label = options.label || "Print PDF";
    options.header = options.header || "Print PDF";
...
/*            var len = options.pages.length;
            ...
            for (; currentPage < len; currentPage++) {            	
            	if (options.pages[ currentPage ] instanceof window.Feedback.Screenshot) {
            		options.pages[ currentPage ].startPDF();
                }
            }
            var promisedOption = options;
            window.setTimeout(function(){
            	currentPage = 0;
            	for (; currentPage < len; currentPage++) {
            		if (promisedOption.pages[ currentPage ] instanceof window.Feedback.Review) {
                    	promisedOption.pages[ currentPage ].render( promisedOption.pages );
                    }
            		if (options.pages[ currentPage ] instanceof window.Feedback.Screenshot) {
                		options.pages[ currentPage ].close();
                		options.pages[ currentPage ].endPrintPDF();
                    }
                }
            }, 500);
        },
...
//        if(pages[ i ] instanceof window.Feedback.Screenshot)
//        pages[ i ].toPDF( pdf );
//    }
//	pdf.save('web.pdf');
//    return this;
...
window.Feedback.Screenshot.prototype.endPrintPDF = function(){
    removeElements( [this.h2cCanvas] );
    this.h2cDone = false;
};
...
window.Feedback.Screenshot.prototype.startPDF = function() {
...
window.Feedback.Screenshot.prototype.toPDF = function( pdf ) {
    ...
    if ( data !== undefined ) {
        ...
        pdf.addImage(data, 'PNG', 0, 100, img.width/2, img.height/2);
    }
